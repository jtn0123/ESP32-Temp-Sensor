...
void SmartRefresh::registerRegion(uint8_t region_id) {
  if (findRegion(region_id))
    return;  // Already registered
  if (region_count_ >= MAX_REGIONS)
    return;
...
void SmartRefresh::resetStats() { stats_ = {}; }

void SmartRefresh::formatStatsJson(char* out, size_t out_size) const {
  float skip_rate =
      stats_.total_checks > 0 ? (100.0f * stats_.skipped_updates / stats_.total_checks) : 0.0f;
