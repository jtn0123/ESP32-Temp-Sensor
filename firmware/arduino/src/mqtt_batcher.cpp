...
void MQTTBatcher::resetStats() { stats_ = {}; }

void MQTTBatcher::formatStatsJson(char* out, size_t out_size) const {
  float avg_batch =
      stats_.flush_count > 0 ? static_cast<float>(stats_.total_flushed) / stats_.flush_count : 0.0f;

  safe_snprintf_rt(out, out_size,
                   "{\"queued\":%u,\"flushed\":%u,\"flushes\":%u,\"drops\":%u,\"avg_batch\":%.1f,"
                   "\"current_queue\":%zu}",
