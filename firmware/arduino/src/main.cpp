...
#if USE_UI_SPEC
// Expansion machinery (template walker, weather shortener) lives in
// spec_render.h so the native_spec_render env can test it host-side.

// Resolve one {placeholder} key from a ui_spec op template using live device
// state, written into the caller's buffer. Shared by every templated op
// (OP_TEXT, OP_TEXTCENTEREDIN) so a template like {weather} or {time_hhmm}
// never prints literally. Allocation-free: this runs once per op per refresh
// and used to churn String temporaries through the heap.
static void spec_format_field_into(const char* key, char* out, size_t n) {
  auto starts = [&](const char* p) { return strncmp(key, p, strlen(p)) == 0; };
  if (strcmp(key, "room_name") == 0) {
    snprintf(out, n, "%s", rc_room_name());
    return;
  }
  if (strcmp(key, "ip") == 0) {
    net_ip_cstr(out, n);
  ...
  // Battery fields ("battery_voltage" prefix-matches its :.2f format spec)
  if (strcmp(key, "battery_percent") == 0) {
    BatteryStatus bs = read_battery_status();
    snprintf(out, n, "%d", bs.percent);
    return;
  }
  if (starts("battery_voltage")) {
    BatteryStatus bs = read_battery_status();
    snprintf(out, n, "%.2f", bs.voltage);
    return;
  }
  if (strcmp(key, "days") == 0) {
    BatteryStatus bs = read_battery_status();
    snprintf(out, n, "%d", bs.estimatedDays);
    return;
  }
  // Inside sensor fields
  if (strcmp(key, "inside_hum_pct") == 0) {
    InsideReadings ir = read_inside_sensors();
    snprintf(out, n, "%.0f", ir.humidityPct);
    return;
  }
  if (starts("pressure_hpa")) {
    InsideReadings ir = read_inside_sensors();
    snprintf(out, n, "%.1f", ir.pressureHPa);
    return;
  }
  // Outside sensor fields
  if (strcmp(key, "outside_hum_pct") == 0) {
    OutsideReadings o = net_get_outside();
    snprintf(out, n, "%.0f", o.humidityPct);
    return;
  }
  // Distinct from the inside "pressure_hpa" case above: a prefix match, so
  // neither key can match the other's branch.
  if (starts("outside_pressure_hpa")) {
    OutsideReadings o = net_get_outside();
    if (!o.validPressure || !isfinite(o.pressureHPa)) {
      snprintf(out, n, "--");
      return;
    }
    snprintf(out, n, "%.0f", o.pressureHPa);
    return;
  }
  if (starts("wind_mps")) {
    OutsideReadings o = net_get_outside();
    snprintf(out, n, "%.1f", o.windMps * 2.237f);
    return;
  }
  if (strcmp(key, "weather_short") == 0) {
    OutsideReadings o = net_get_outside();
    snprintf(out, n, "%s", spec_short_condition(o.weather));
    return;
  }
  if (strcmp(key, "weather") == 0) {
    OutsideReadings o = net_get_outside();
    snprintf(out, n, "%s", o.weather);
    return;
  }
  if (strcmp(key, "time_hhmm") == 0) {
    net_time_hhmm(out, n);
    return;
  }
  if (strcmp(key, "inside_temp_f") == 0) {
    InsideReadings ir = read_inside_sensors();
    if (!isfinite(ir.temperatureC)) {
      snprintf(out, n, "--");
      return;
    }
    snprintf(out, n, "%.0f", ir.temperatureC * 9.0f / 5.0f + 32.0f);
    return;
  }
  if (strcmp(key, "outside_temp_f") == 0) {
    OutsideReadings o = net_get_outside();
    if (o.validTemp && isfinite(o.temperatureC)) {
      snprintf(out, n, "%.0f", o.temperatureC * 9.0f / 5.0f + 32.0f);
      return;
    }
    if (isfinite(get_last_outside_f())) {
      snprintf(out, n, "%.0f", get_last_outside_f());
      return;
    }
    snprintf(out, n, "--");
    return;
  ...
  if (starts("hist_")) {
    ...
    float mn = 0, mx = 0;
    bool temp_group = starts("hist_temp");
    ...
    bool is_max = klen >= 4 && strcmp(key + klen - 4, "_max") == 0;
    snprintf(out, n, "%d", static_cast<int>(is_max ? (mx + 0.5f) : mn));
    return;
  }
  if (strcmp(key, "date_mmmdd") == 0) {
    // "Jul 30" - the date half of the header's last-updated stamp.
    time_t now = time(nullptr);
    struct tm tm_now;
    localtime_r(&now, &tm_now);
    strftime(out, n, "%b %d", &tm_now);
    return;
  }
  snprintf(out, n, "--");
}
