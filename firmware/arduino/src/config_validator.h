...
inline ErrorCode validate_config() {
  ...
  if (WAKE_INTERVAL_SEC < MIN_WAKE_INTERVAL_SEC || WAKE_INTERVAL_SEC > MAX_WAKE_INTERVAL_SEC) {
    LOG_ERROR("Config: wake_interval out of range: %d (must be %d-%d)", WAKE_INTERVAL_SEC,
              MIN_WAKE_INTERVAL_SEC, MAX_WAKE_INTERVAL_SEC);
  ...
  if (FULL_REFRESH_EVERY < MIN_FULL_REFRESH || FULL_REFRESH_EVERY > MAX_FULL_REFRESH) {
    LOG_ERROR("Config: full_refresh_every out of range: %d (must be %d-%d)", FULL_REFRESH_EVERY,
              MIN_FULL_REFRESH, MAX_FULL_REFRESH);
    return ERR_CONFIG_OUT_OF_RANGE;
  }
  ...
  if (MQTT_PORT < MIN_MQTT_PORT || MQTT_PORT > MAX_MQTT_PORT) {
    LOG_ERROR("Config: MQTT port out of range: %d (must be %d-%d)", MQTT_PORT, MIN_MQTT_PORT,
              MAX_MQTT_PORT);
    return ERR_CONFIG_OUT_OF_RANGE;
  }
...
inline ErrorCode startup_config_check() {
  ...
  if (err != ERR_NONE) {
    LOG_ERROR("Configuration validation failed with error %d: %s", err, error_to_string(err));
