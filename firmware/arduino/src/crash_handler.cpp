...
void CrashHandler::formatCrashReport(char* out, size_t out_size) const {
  snprintf(out, out_size,
           ...
           "\"heap_at_crash\":%u,"
           "\"min_heap\":%u}",
           crash_info_.crash_count, crash_info_.boot_count, crash_info_.last_crash_timestamp,
           getResetReasonString(crash_info_.reset_reason),
           getRtcResetReasonString(crash_info_.rtc_reset_reason), crash_info_.last_function,
           crash_info_.free_heap_at_crash, crash_info_.min_free_heap);
}
