...
{OP_BATTERYGLYPH, 255, 0, 0, 8, 88, 13, 7, "battery_percent", \
 NULL},
{ OP_TEXT, 255, 2, 0, 26, 86, 0, 0,
  "Batt {battery_voltage:.2f}V {battery_percent}%", NULL },
...
const int kOps_footer_split_count = \
sizeof(kOps_footer_split)/sizeof(kOps_footer_split[0]);
...
const int kVariant_v2_ops_count = \
sizeof(kVariant_v2_ops)/sizeof(kVariant_v2_ops[0]);

const ComponentOps* get_variant_ops(uint8_t variantId, int* outCount) {
  if (variantId == 0) {
if (outCount) {
  *outCount = kVariant_v2_ops_count;
}
return kVariant_v2_ops;
  }
  if (outCount) {
*outCount = 0;
  }
