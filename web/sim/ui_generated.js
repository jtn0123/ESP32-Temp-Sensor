  ...
  function uiMapWeather(iconMap, weather){
    ...
    for (const rule of iconMap){
      if (rule && Array.isArray(rule.match)){
        for (const m of rule.match){ if (s.includes(String(m).toLowerCase())) return rule.icon; }
      } else if (rule && rule.default) { return rule.icon; }
    }
    return null;
  }
  try { window.uiMapWeather = (w)=>uiMapWeather((window.UI_SPEC||{}).iconMap, w); } catch(e){}
)();
