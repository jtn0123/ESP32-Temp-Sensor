...
CONFIG_FILES = [
    ...
    {"path": "config/icons", "schema": "icons_directory", "required": False},
]
...
DEVICE_CONFIG_SCHEMA = {
    ...
    "optional_fields": ["fw_version", "ha_entities", "thresholds", "battery", "active_seconds"],
    "wake_interval_values": ["1h", "2h", "4h"],
    "outside_source_values": ["mqtt", "ha"],
    "full_refresh_every_range": (1, 100),
    "room_name_constraints": {"max_length": 32, "disallowed_chars": set("<>:/\\|?*#")},
}
...
UI_SPEC_SCHEMA = {
    ...
    "canvas_required": ["w", "h"],
    "canvas_constraints": {"w": {"min": 100, "max": 500}, "h": {"min": 50, "max": 300}},
}
...
def _validate_device_config_schema(config: Dict[str, Any], file_path: str):
    ...
    assert (
        len(room_name) <= DEVICE_CONFIG_SCHEMA["room_name_constraints"]["max_length"]
    ...
    if isinstance(wake_interval, str):
        assert (
            wake_interval in DEVICE_CONFIG_SCHEMA["wake_interval_values"]
    ...
    range_tuple = DEVICE_CONFIG_SCHEMA["full_refresh_every_range"]
    min_val, max_val = range_tuple[0], range_tuple[1]
    assert (
        min_val <= full_refresh <= max_val
    ...
    assert (
        outside_source in DEVICE_CONFIG_SCHEMA["outside_source_values"]
...
def _validate_ui_spec_schema(ui_spec: Dict[str, Any]):
    ...
    assert (
        w_constraints["min"] <= canvas_w <= w_constraints["max"]
    ), f"Canvas width {canvas_w} out of range"
    assert (
        h_constraints["min"] <= canvas_h <= h_constraints["max"]
    ...
    for rect_name, rect_coords in rects.items():
        ...
        _x, _y, w, h = rect_coords[0], rect_coords[1], rect_coords[2], rect_coords[3]
        assert all(
            isinstance(coord, int) for coord in rect_coords
...
def test_display_geometry_consistency():
    ...
    if os.path.exists(ui_spec_path) and os.path.exists(geom_path):
        ...
        if ui_canvas and geom_canvas:
            assert ui_canvas.get("w") == geom_canvas.get(
                "w"
            ), "Canvas width mismatch between UI spec and display geometry"
            assert ui_canvas.get("h") == geom_canvas.get(
                "h"
            ...
            if wifi:
                ...
                if "static" in wifi:
                    ...
                    for field in required_static:
                        ...
                        assert static[
                            field
                        ].strip(), f"Empty static IP field '{field}' in {config_path}"
