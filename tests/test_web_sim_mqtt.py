...
from scripts.mqtt_topics import build_discovery_topic, build_topic, format_sensor_value
...
def test_simulator_discovery_format(mosquitto_broker):
    ...
    publisher = SimulatorMQTTPublisher(
        broker=host, port=port, device_id=device_id, room_name=room_name
    )
...
def test_simulator_availability_not_retained(mosquitto_broker):
    ...
    publisher = SimulatorMQTTPublisher(
        broker=host, port=port, device_id=device_id, room_name="Test"
    )
