...
try:
    ...
    # Skip (not sys.exit) so a missing optional dep doesn't abort the whole
    # pytest run with an INTERNALERROR during collection.
    pytest.skip(
        f"Missing dependency: {e} (install with: pip install numpy pillow playwright)",
