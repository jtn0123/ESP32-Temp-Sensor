    ...
    if os.path.exists(gen_icons_script):
        try:
            result = subprocess.run([
                "python3", gen_icons_script,
                "--config", os.path.join(os.path.dirname(scripts_dir), "config", "icons")
    ...
    if os.path.exists(convert_script):
        try:
            result = subprocess.run([
                ...
                "--input", os.path.join(os.path.dirname(scripts_dir), "web", "icons"),
                "--output", "/tmp/test_icons.h"
            ], capture_output=True, text=True, cwd=scripts_dir, timeout=30)
...
def test_icon_naming_conventions():
    ...
    for icon_name in all_icons:
        ...
        assert icon_name.replace('-', '').replace('_', '').isalnum(), \
...
def test_weather_icon_mapping_completeness():
    ...
    essential_conditions = {
        "clear", "sunny", "partly", "cloud", "overcast",
