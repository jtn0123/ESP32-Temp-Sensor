...
#### ~~F1 — Plan the arduino-esp32 3.x migration~~ ✓ done 2026-07-31 (`4cb4ab5`, `bda18a2`, `2dfce62`) — *pioarduino 55.03.311 / core 3.3.11 / IDF 5.5.5. Bench-verified on hardware: USB flash v2.00, then v2.01 OTA over the 3.x stack; WiFi/NTP/mDNS/MQTT/HA discovery/screenshots/page-flip/NeoPixel all green; R3 MSC hardware-confirmed in passing. Two real 3.x bugs found+fixed at the bench (efuse client id, dotenv-less .env parsing). On-battery recheck pending unplug: fuel gauge (nan on one warm reboot), FFat/backfill (host held the disk).*
