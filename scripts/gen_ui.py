...
def emit_web_js(spec: Dict[str, Any]) -> str:
    ...
    # Use firmware FW_VERSION for simulator parity when available.
    # Prefer the generated header (emitted by gen_device_header.py), else fall back to git info.
    fw_version = ""
    gen_cfg = FW_OUT_DIR / "generated_config.h"
    try:
        txt = gen_cfg.read_text()
...
def main() -> None:
    ...
    print(f"Wrote {WEB_SIM_GEOM_JSON.relative_to(ROOT)}")
