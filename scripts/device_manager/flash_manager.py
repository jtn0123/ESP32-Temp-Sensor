...
class QueuedFlash:
    ...
    status: str = (
        "pending"  # pending, building, hunting, flashing, completed, failed, cancelled, expired
    )
...
class FlashManager:
    ...
    async def flash(
        self, port: str, firmware_path: Optional[str] = None, build_config: str = "dev"
    ) -> bool:
        """
        ...
        try:
            ...
            env_map = {
                ...
                "prod": (
                    self.config.platformio_env_prod
                    if self.config
                    else "feather_esp32s2_display_only"
                ),
            ...
            if not firmware_path:
                ...
                if not build_success:
                    ...
                    await self._broadcast(
                        "flash_complete", {"success": False, "message": error_msg}
                    )
                    return False
            ...
            await self._broadcast(
                "flash_progress",
                {"percent": 50, "stage": "flashing", "message": "Flashing firmware..."},
            )
            ...
            await self._broadcast(
                "flash_complete", {"success": False, "message": f"Error: {str(e)}"}
            )
            return False
            ...
            async for line in process.stdout:
                ...
                if text:
                    ...
                    if percent is not None:
                        ...
                        await self._broadcast(
                            "flash_progress",
                            {"percent": 75, "stage": "writing", "message": text[:100]},
                        )
    ...
    async def cancel(self):
        ...
        if self.process and self.process.poll() is None:
            ...
            await self._broadcast(
                "flash_complete", {"success": False, "message": "Flash cancelled by user"}
            )
    ...
    async def queue_flash(
        ...
        env_map = {
            "dev": self.config.platformio_env_dev if self.config else "dev_display",
            "prod": (
                self.config.platformio_env_prod if self.config else "feather_esp32s2_display_only"
            ),
            "battery_test": "feather_esp32s2_battery_test",
        }
        ...
        if not build_success:
            ...
            await self._broadcast(
                "flash_queue_status",
                {"status": "failed", "message": error_msg, "queue": self.queued_flash.to_dict()},
            )
        ...
        while self._monitoring and self.queued_flash is not None:
            try:
                ...
                if self.queued_flash and self.queued_flash.is_expired():
                    ...
                    if self._loop:
                        asyncio.run_coroutine_threadsafe(self._handle_queue_expired(), self._loop)
                    break
                    ...
                    if self._port_matches_target(port):
                        ...
                        if self._loop:
                            asyncio.run_coroutine_threadsafe(
                                self._handle_device_detected(port), self._loop
                            )
    ...
    def on_mdns_device_found(self, device_id: str, ip_address: str):
        ...
        if self.queued_flash.target_device_id == device_id:
            ...
            if self._loop:
                asyncio.run_coroutine_threadsafe(
                    self._handle_ota_device_detected(device_id, ip_address), self._loop
                )
        ...
        try:
            ...
            async for line in process.stdout:
                ...
                if text:
                    ...
                    await self._broadcast(
                        "flash_progress",
                        {"percent": 75, "stage": "ota_upload", "message": text[:100]},
                    )
