...
class DeviceListener(ServiceListener):
    ...
    def remove_service(self, zc: Zeroconf, type_: str, name: str) -> None:
        ...
        with self._lock:
            ...
            device_name = name.replace(f".{SERVICE_TYPE}", "").replace(
                "._espsensor._tcp.local.", ""
            )
    ...
    def _process_service_info(self, name: str, info: ServiceInfo) -> None:
        ...
        with self._lock:
            ...
            if is_new:
                logger.info(
                    f"Device discovered: {device.room} ({device.ip_address}) v{device.version}"
                )
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self._listener = DeviceListener(
                on_change=self.on_devices_changed, on_device_added=self.on_device_added
            )
