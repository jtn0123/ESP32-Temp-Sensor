...
class SerialManager:
    ...
    def list_ports(self) -> List[Dict[str, Any]]:
        ...
        for port in serial.tools.list_ports.comports():
            ...
            is_esp32 = any(
                chip in description for chip in ["cp210", "ch340", "ftdi", "usb serial", "uart"]
            )
    ...
    def connect(self, port: str, baud: int = 115200) -> bool:
        ...
        try:
            ...
            self.serial = serial.Serial(port=port, baudrate=baud, timeout=1.0, write_timeout=1.0)
            ...
            if self.hub:
                asyncio.create_task(
                    self.hub.broadcast(
                        {"type": "serial_status", "connected": True, "port": port, "baud": baud}
                    )
                )
    ...
    def disconnect(self):
        ...
        if self.hub:
            asyncio.create_task(self.hub.broadcast({"type": "serial_status", "connected": False}))
    ...
    def _process_line(self, line: str):
        ...
        if self.hub:
            message = {"type": "serial", "data": line, "timestamp": time.time()}
