...
def draw_layout(draw: ImageDraw.ImageDraw, data: dict):
    ...
    # left-justify numeric; place degree and F immediately after the number
    num_in = str(data.get('inside_temp','72.5'))
    draw.text((INSIDE_TEMP[0], INSIDE_TEMP[1]), num_in, font=font_big, fill=0)
    nwi = len(num_in)*6
    draw.text((INSIDE_TEMP[0]+nwi+2, INSIDE_TEMP[1]+2), "°", font=load_font(10), fill=0)
    draw.text((INSIDE_TEMP[0]+nwi+8, INSIDE_TEMP[1]+2), "F", font=load_font(10), fill=0)
