...
def main() -> int:
	...
	try:
		...
		with sync_playwright() as p:
			...
			page.on("requestfailed", lambda r: fails.append(f"FAIL {r.method} {r.url} {r.failure.error_text if r.failure else ''}"))
			url = f"http://127.0.0.1:{port}/index.html?spec=1&v=4"
			page.goto(url, wait_until="load")
			page.wait_for_timeout(700)
			scripts = page.evaluate("()=>Array.from(document.querySelectorAll('script')).map(s=>s.src||'inline')")
			...
			for s in scripts:
				print(" ", s)
			print("UI_SPEC:", ui_spec_type, "draw:", draw_type, "drawGen:", draw_gen_type, "pixel[5,18]:", tuple(pix))
