...
JSON_REL = "config/display_geometry.json"
...
def generate_header(data: Dict[str, Any]) -> str:
    ...
    lines.append(f"// source: {JSON_REL}")
...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate display_layout.h from display geometry JSON")
    p.add_argument("--geometry", help=f"path to the geometry JSON (default: {JSON_REL})")
    p.add_argument(
        "--out",
        help="path to write the generated header (default: firmware/arduino/src/display_layout.h)",
    )
    ...
    if unknown:
        print(
            f"WARN: gen_layout_header.py ignoring unrecognized arguments: {unknown}",
            file=sys.stderr,
        )
    return args
...
def main(argv: List[str] | None = None) -> None:
    ...
    json_path = pathlib.Path(args.geometry).resolve() if args.geometry else JSON_PATH
    out_path = pathlib.Path(args.out).resolve() if args.out else OUT_PATH
