            ...
            if metrics and metrics.get('text'):
                ...
                if text_width > rect_width:
                    ...
                    issues.append(ValidationIssue(
                        ...
                        severity=severity,
                        region=region.name,
                        description=f"Text overflows horizontally by {overflow_px:.1f}px ({overflow_pct:.1f}%)",
        ...
        if np.any(content_mask):
            ...
            if min_row < edge_threshold:
                issues.append(ValidationIssue(
                    ...
                    region=region.name,
                    description=f"Content appears clipped at top edge",
            ...
            if max_row >= h - edge_threshold:
                issues.append(ValidationIssue(
                    ...
                    severity=ValidationSeverity.ERROR,
                    region=region.name,
                    description=f"Content appears clipped at bottom edge",
                    coordinates=(x + min_col, y + min_row, content_width, content_height)
                ))
            ...
            if min_col < edge_threshold:
                issues.append(ValidationIssue(
                    ...
                    severity=ValidationSeverity.ERROR,
                    region=region.name,
                    description=f"Content appears clipped at left edge",
                    coordinates=(x + min_col, y + min_row, content_width, content_height)
                ))
            ...
            if max_col >= w - edge_threshold:
                issues.append(ValidationIssue(
                    ...
                    severity=ValidationSeverity.ERROR,
                    region=region.name,
                    description=f"Content appears clipped at right edge",
                    coordinates=(x + min_col, y + min_row, content_width, content_height)
                ))
